...
class Player(pygame.sprite.Sprite):
    ...
    def imgchange(self, image_path):
        self.image = pygame.image.load(image_path).convert_alpha() # convert_alpha preserves transparency, improves performance  
        self.image = pygame.transform.scale(self.image,(50,50))
...
def show_score(score, color=BLACK, x=10,y=20):
    ...
    screen.blit(high_score_text,(x,60))
...
def produce_positions(POSITION_COUNT):
    list = []
    for i in range(POSITION_COUNT):
        list.append(round(i*330/(POSITION_COUNT-1),1))
    return list 
...
def main():
    ...
    for i in range(7):
        ...
        choices = random.sample(produce_positions(POSITION_COUNT),2)
